...
Now, there needs to be a feature coded into the game that makes these statements true. If all the statements in a game's design are true, it should be the case that the game works as intended and is enjoyable to play. In the above case this would result in a shooter where positioning, strategy and not getting outmaneuvered are the most important things.
